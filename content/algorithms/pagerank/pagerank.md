...
Markov chains are mathematical models that represent systems that evolve over time in a probabilistic manner. These systems consist of a set of states, and transitions between these states occur randomly based on specific probabilities.

The key characteristic of Markov chains is that the probability of transitioning from one state to another depends solely on the current state and is independent of the past states.

Markov chains can be used to model the web network, particularly in the context of the PageRank algorithm.
...
In the above plot, we model a simple Markov chain with 4 states. The transition probability is simply based on the outgoing links assuming that user is equally likely to transition to all those states i.e. if a state has 3 outgoing links, the transition probability of each of those will be 0.33.
...
- Problem of sink states in real world web networks
- how to solve the problem using random state transition/teleportation probabilites and damping factor
- also discuss topic specific/personalized pagerank
