...
degree_colors = [
    color_map_color(degrees[i]) for i in range(34)
]  # Translate the degree value of each node into a value
...
G = nx.barbell_graph(
    3, 1
)  # Two complete graphs of 3 nodes connected with a path of 1 node
...
print(
    "Isolated nodes in group Administrator ", len(K1_naive.nodes) - len(K1_not_isolated)
)
...
print("Karate club graph has: ", len(K.edges), " edges")
print(
    "After the partition there are ",
    len(K2_real.edges) + len(K1_real.edges),
    " edges left",
)
```
...
```{code-cell}
print(
    "Are isomorphic the graphs corresponding to the administrator? ",
    nx.is_isomorphic(K1, K1_real),
)
print(
    "Are isomorphic the graphs corresponding to the instructor? ",
    nx.is_isomorphic(K2, K2_real),
)
```
...
print(
    "Naive vs fiedler vector correlation: ",
    abs(np.corrcoef(partition_fiedler, partition_naive)[0, 1]),
)
print("Real vs naive correlation: ", abs(np.corrcoef(real_cut, partition_naive)[0, 1]))

print(
    "Real vs fiedler vector correlation: ",
    abs(np.corrcoef(real_cut, partition_fiedler)[0, 1]),
)
```
